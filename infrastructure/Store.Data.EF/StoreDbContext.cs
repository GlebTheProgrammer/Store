...
namespace Store.Data.EF
{
    public class StoreDbContext : DbContext
    {
        ...
        {
            // Будем для BookDto описывать параметры свойств, то есть будет описана таблица, в таблице будут книги
        ...
        private static void BuildOrders(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<OrderDto>(action =>
            {
                ...
                // Добавляем свойство и говорим, что у него есть конверсия, он берёт на вход словарь, что-то с ним сделает, и вернёт строку
                // но ему для работы нужен ValueComparer. Для того, чтобы Entity Framework должен проверять словари на совпадение, мы
        ...
        // Тут 2 метода. Первый который сравнивает две последовательности, а второй умеет считать HashCode
